...
import { useContent, useUpdateContent, useContentAction, useDeleteContent } from '@/lib/hooks/useContents'
...
export default function ContentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const variants = (content.captionVariants as any[]) ?? []
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* Product info */}
      {content.productData && Object.keys(content.productData as any).length > 0 && (
        <div style={{
          marginTop: '20px',
          padding: '16px 20px',
          background: '#F8FAFC',
          ...
          <dl style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '6px 14px', fontSize: '12px' }}>
            {(content.productData as any).name && (
              <>
                <dt style={{ color: '#64748B' }}>Nama:</dt>
                <dd style={{ color: '#0F172A', fontWeight: 500 }}>{(content.productData as any).name}</dd>
              </>
            )}
            {(content.productData as any).price && (
              <>
                <dt style={{ color: '#64748B' }}>Harga:</dt>
                <dd style={{ color: '#0F172A', fontWeight: 500 }}>{(content.productData as any).price}</dd>
              </>
            )}
          </dl>
        </div>
