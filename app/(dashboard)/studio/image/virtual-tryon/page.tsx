...
// ── Design tokens ─────────────────────────────────────────────
const T = {
  bg:       '#FCFBF7',         // Warm organic honey white backdrop
  surface:  'rgba(255, 255, 255, 0.65)', // Pure glass honeycomb base
  card:     'rgba(245, 158, 11, 0.03)',  // Ambient honey nectar fill
  border:   'rgba(15, 23, 42, 0.08)',
  borderHi: 'rgba(245, 158, 11, 0.25)', // Honey focus line
  accent:   '#2563EB',         // Royal Core Blue
  accent2:  '#F59E0B',         // Deep Amber Honey
  gold:     '#D97706',         // Burnished Bronze Gold
  green:    '#059669',
  red:      '#DC2626',
  muted:    '#64748B',
  dimmed:   '#475569',
  sub:      '#334155',
  text:     '#0F172A',         // Deep high-contrast tech slate text
  white:    '#FFFFFF',
  g1: 'linear-gradient(135deg, #F59E0B 0%, #D97706 100%)', // Pure Honey Swirl Gradient
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: `${accent}12`, border: `1px solid ${accent}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px' }}>{icon}</div>
        <div>
          <div style={{ fontSize: '13px', fontWeight: 700, color: T.text }}>{label}</div>
          <div style={{ fontSize: '10px', color: T.muted }}>{sublabel}</div>
        </div>
      </div>
      ...
      <div
        ...
        style={{
          position: 'relative',
          borderRadius: '20px',
          border: `1.5px dashed ${drag ? accent : preview ? 'transparent' : 'rgba(15, 23, 42, 0.12)'}`,
          background: drag ? `${accent}08` : 'rgba(255, 255, 255, 0.8)',
          ...
          boxShadow: preview ? '0 12px 30px rgba(15,23,42,0.06)' : 'none'
        }}
        className="upload-interactive-node"
      >
        {preview ? (
          <>
            <img src={preview} alt={label} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            <div style={{ position: 'absolute', inset: 0, background: 'rgba(15, 23, 42, 0)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', transition: 'all 0.3s' }} className="overlay-hover-layer">
              <button onClick={e => { e.stopPropagation(); ref.current?.click() }}
                style={{ padding: '8px 16px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.4)', background: 'rgba(255,255,255,0.3)', color: '#fff', fontSize: '11px', fontWeight: 700, cursor: 'pointer', backdropFilter: 'blur(12px)', transition: 'all 0.2s', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
              >
                Ganti Gambar
              </button>
              <button onClick={e => { e.stopPropagation(); onClear() }}
                style={{ width: '34px', height: '34px', borderRadius: '50%', border: 'none', background: T.red, color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(220,38,38,0.3)', transition: 'all 0.2s' }}
              >
                <X size={14} />
              </button>
            </div>
            <div style={{ position: 'absolute', top: '10px', left: '10px', padding: '4px 10px', borderRadius: '8px', background: T.green, color: '#fff', fontSize: '10px', fontWeight: 700, boxShadow: '0 4px 10px rgba(5,150,105,0.2)' }}>
              ✓ Node Siap
            </div>
          </>
        ) : (
          <div style={{ textAlign: 'center', padding: '24px' }}>
            <div style={{ width: '54px', height: '54px', borderRadius: '18px', background: `${accent}08`, border: `1px solid ${accent}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', margin: '0 auto 14px', color: accent }}>
              {icon}
            </div>
            <div style={{ fontSize: '14px', fontWeight: 700, color: T.text, marginBottom: '6px' }}>
              {drag ? 'Lepaskan Gambar Produk' : 'Unggah Aset Foto'}
            </div>
            <div style={{ fontSize: '11px', color: T.muted, lineHeight: 1.5, marginBottom: '16px' }}>
              Drag & drop atau klik bilah sel ini
            </div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 18px', borderRadius: '10px', background: accent, color: '#fff', fontSize: '12px', fontWeight: 700, boxShadow: `0 6px 16px ${accent}25` }}>
      ...
      {tips && !preview && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', background: 'rgba(15, 23, 42, 0.02)', padding: '12px', borderRadius: '14px', border: `1px solid ${T.border}` }}>
          {tips.map((t, i) => (
            <div key={i} style={{ display: 'flex', gap: '6px', alignItems: 'flex-start' }}>
              <div style={{ width: '5px', height: '5px', borderRadius: '50%', background: accent, marginTop: '6px', flexShrink: 0 }} />
              <span style={{ fontSize: '11px', color: T.dimmed, lineHeight: 1.5 }}>{t}</span>
            </div>
          ))}
        </div>
      )}
...
function ResultCompare({ before, after }: { before: string; after: string }) {
  ...
  return (
    <div ref={ref}
      style={{ position: 'relative', width: '100%', aspectRatio: '3/4', borderRadius: '20px', overflow: 'hidden', cursor: 'col-resize', userSelect: 'none', background: '#F1F5F9', border: `1px solid ${T.border}` }}
...
function LoadingState({ message }: { message: string }) {
  const steps = [
    'Menganalisis matriks & pose tubuh model...',
    'Mengekstrak jalinan corak serat pakaian...',
    'Menyelaraskan luminansi cahaya lebah...',
    'AI merajut fitting kain ke lekukan tubuh...',
    'Finalisasi katalog digital resolusi tinggi...',
  ]
      ...
      <div style={{ maxWidth:'1280px', margin:'0 auto', padding:'24px 20px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'16px', alignItems:'flex-start' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            ...
            {/* Error */}
            {error && (
              <div style={{ padding: '12px', borderRadius: '14px', background: 'rgba(220,38,38,0.08)', border: `1px solid rgba(220,38,38,0.2)`, display: 'flex', gap: '8px', alignItems: 'center' }}>
                <AlertCircle size={14} color={T.red} style={{ flexShrink: 0 }} />
        ...
        {/* ── Tips bar ────────────────────────────────────── */}
        <div style={{ marginTop: '24px', padding: '18px', borderRadius: '24px', background: T.surface, border: `1px solid ${T.border}`, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px', backdropFilter: 'blur(20px)' }} className="luxury-glass-card">
          {[
            ...
            { tag: '💡 Prompt Tip', text: 'Tulis instruksi dalam bahasa Inggris sederhana hanya untuk gaya pakai (misal: "rolled sleeves"). Jangan sebut warna produk.' },
            { tag: '🎯 Akurasi Gambar', text: 'Foto pakaian flat-lay di lantai/meja berlatar polos sangat dianjurkan jika Anda tidak memiliki patung manekin.' },
          ].map((tip,i) => (
            <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
              ...
              <div>
                ...
                {tip.tag && (
                  <div style={{ padding: '4px 8px', borderRadius: '6px', background: 'rgba(255,255,255,0.04)', border: `1px solid ${T.borderHi}`, fontSize: '10px', fontWeight: 700, color: T.gold, whiteSpace: 'nowrap' }}>{tip.tag}</div>
                )}
                {tip.text && (
                  <div style={{ fontSize: '12px', color: T.dimmed, lineHeight: 1.5 }}>{tip.text}</div>
                )}
              </div>
            </div>
          ))}
      ...
      <style>{`
        * { box-sizing: border-box; }
        .cyber-studio-canvas {
          ...
          animation: studioGridShift 30s linear infinite;
        }
        @keyframes studioGridShift {
        ...
        .glass-card-wrapper:hover .luxury-glass-card {
          transform: translateY(-2px);
          box-shadow: 0 20px 40px -10px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.05);
        }
        .upload-interactive-node:hover .overlay-hover-layer {
          background: rgba(9, 10, 15, 0.65) !important;
        }
        .upload-interactive-node:hover .overlay-hover-layer button {
          opacity: 1 !important;
        }
        .btn-luxury-submit:hover:not(:disabled) {
          transform: translateY(-1.5px);
          filter: brightness(1.06);
          box-shadow: 0 12px 30px -4px rgba(37,99,235,0.5) !important;
        }
        .link-hover-effect:hover {
          color: ${T.white} !important;
          text-decoration: underline !important;
        }
        .starter-card-hover:hover {
